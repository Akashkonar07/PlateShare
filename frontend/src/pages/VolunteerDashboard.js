...
const VolunteerDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Filter small/medium donations that are pending
  ...
  const handleAction = async (donationId, action) => {
    try {
      await updateDonationStatus(donationId, action, token);
      setDonations(prev => prev.map(d => d._id === donationId ? { ...d, status: action } : d));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      {donations.length === 0 && !loading && <p>No nearby donations at the moment.</p>}
      <div className="grid md:grid-cols-2 gap-4">
        {donations.map(d => (
          <DonationCard key={d._id} donation={d} onAction={handleAction} />
        ))}
      </div>
